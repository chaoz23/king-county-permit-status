...
- [ ] **A1 · Recon spike (blocks all counts below).** Follow each Tier-1 city's real permit-portal link (browser, not curl — several block plain requests / are JS SPAs) and record vendor + whether a search API is reachable. Output: city→vendor→reachable matrix. Until this lands, A3+ counts are estimates. `[M]`
  ...
  **Vendor fingerprint sweep (2026-07-17) — partial map + a hard boundary.** Ran a fetch-based fingerprint pass over the fallback frontier. Confirmed vendors:

  | City | Vendor | Portal / evidence | Reuse? |
  |---|---|---|---|
  | Shoreline | **eTRAKiT** | `permits.shorelinewa.gov/eTRAKiT` | done (A3, live) |
  | Maple Valley | **Tyler EnerGov** | `tyler` marker; portal link JS-only | reuses A2 (EnerGov) |
  | King County | **Accela** | `aca-prod.accela.com/kingco` | new adapter (A4) |
  | Tukwila | **ASP.gov** (migrated *off* eTRAKiT) | `tukw-egov.aspgov.com/BP/`; public search | new adapter |
  | Kent | dedicated Citizen Portal | `permitstatus.kentwa.gov` (blocks curl) | TBD |
  | Duvall | **OpenGov** | homepage marker | new adapter |
  | Carnation | **SmartGov** | homepage marker | new adapter |
  | Redmond, Covington, Des Moines, SeaTac, Enumclaw, North Bend, Woodinville, Black Diamond | unknown | portal/homepage blocks plain HTTP **or** is JS-rendered | needs browser |

  **Two conclusions:** (1) **No free eTRAKiT reuse** — Shoreline is the only current-eTRAKiT KC city found; Tukwila migrated off it. The adapter is the pattern, not a multi-city unlock. (2) The frontier is genuinely **fragmented multi-vendor** (EnerGov, Accela, OpenGov, SmartGov, ASP.gov, eTRAKiT — each 1–2 cities), confirming the adapter-count-driven restructuring. **Fetch-based recon is now exhausted**: the remaining ~9 cities need browser automation to fingerprint the portal and confirm a reachable search API. That is the gate on any further A-series counts.

  **Crystallization economics (2026-07-17).** The winning pattern is: use a browser *once* to capture the portal's underlying request (the XHR/API or the form POST), then bake that into a plain-HTTP `search_<city>` so users pay zero navigation cost — exactly how Renton (EnerGov `/api/energov/search`) and Shoreline (eTRAKiT CSV export) already work. But the recon cost only amortizes when the vendor is **shared across cities**. Priority ≈ (city value) × (vendor reuse count) ÷ (recon difficulty). Practical order when a browser is available:
  - **Highest leverage:** EnerGov cities (reuse Renton's client via **A2**) and Accela cities (King County + others) — one capture, many cities.
  - **Skip / deprioritize:** single-city rare vendors. **Tukwila (ASP.gov)** was probed to 90% over plain HTTP (session cookies, CSRF token, `searchBy`/`parcel.siteStreetNumber`+`siteStreetName`/`parcel.siteApn` fields all mapped), but its results page (`selectpermitresults.html`) redirects to **User Login** — results are auth-gated — and ASP.gov appears on no other KC city. Not worth cracking. Left as fallback.

  **Environment note:** this session's in-app browser was non-functional (navigation timed out even on known-good hosts), and the high-value portals TLS-fingerprint-block plain HTTP (**Redmond** → `SSLV3_ALERT_HANDSHAKE_FAILURE`; **Kent** `permitstatus.kentwa.gov` → timeout). So the high-leverage captures (EnerGov/Accela) should be done in an **interactive session with a working browser surface** (real Chrome), targeting shared vendors first.
...
- [x] **A3 · TRAKiT adapter — Shoreline (DONE 2026-07-17).** Added `search_shoreline` (eTRAKiT/CentralSquare) to `lookup.py`: GET the search page for a `__VIEWSTATE`, then POST with the grid's Export-to-Excel action → all rows as CSV in one request (no pagination). Wired into the router for Shoreline addresses + all permit/parcel searches, and dropped from the fallback-note path. Scorecard flips **Shoreline 🟡→🟢** (first fallback→live conversion; coverage now **15/39 live**). Electrical stays via L&I (Shoreline doesn't self-run it). `status`/`description` come back empty — the eTRAKiT search/export doesn't expose them; only the per-permit detail page (a postback) does. Tests: `ShorelineSearchTests` (6, network-free); suite 138 green. **Follow-ups:** (1) status/description enrichment via the detail postback; (2) legacy bare-numeric permit numbers (e.g. `101014`) aren't classified as permits by `detect_input_type` — modern formats (`ROW26-0686`) route fine. The adapter is now the template for any other eTRAKiT city A1 finds.

  **A1 recon — Shoreline eTRAKiT (done 2026-07-17): GO, lightweight fetch-and-parse, no browser, no login.**
  - **Vendor:** CentralSquare **eTRAKiT** (ASP.NET WebForms + Telerik RadGrid), public.
  - **Endpoint:** `POST https://permits.shorelinewa.gov/eTRAKiT/Search/permit.aspx`. Flow = GET the page, echo back `__VIEWSTATE` + `__VIEWSTATEGENERATOR`, then POST. **No `__EVENTVALIDATION` required.** Stateful GET-then-POST, same shape as our MBP token / Renton criteria patterns.
  - **Form fields:** `ctl00$cplMain$ddSearchBy` (axis) · `ctl00$cplMain$ddSearchOper` (CONTAINS/EQUALS/BEGINS WITH) · `ctl00$cplMain$txtSearchString` (value) · `ctl00$cplMain$btnSearch=SEARCH`.
  - **Search axes match our schema:** address `Permit_Main.SITE_ADDR` · permit# `Permit_Main.PERMIT_NO` · parcel `Permit_Main.SITE_APN` (+ owner/applicant/contractor/type/dates).
  - **Results:** RadGrid, **10 rows/page, paginated** ("page 1 of 10"); rows anchored by `id="ctl00_cplMain_rgSearchRslts_ctl00__N"`. Verified real records back to **2001** (e.g. `101014 | 01/26/2001 | 02/02/2001 | FIRE SYSTEM | 1826049268 | 15332 AURORA AVE N`).
  - **Columns available from the grid:** permit_number, applied_date, issued_date, type, parcel, address, owner, applicant, contractor, RECORDID. Dates are `MM/DD/YYYY` → normalize to ISO.
  - **Open items for the A3 build (not blockers):** (1) **status / description / finaled+expires dates are NOT in the grid** — they live on the permit detail page, which is `__doPostBack`-driven (direct GET by recordid errors); options: drive the detail postback, use the grid's `btnExportToExcel`, or ship grid-only with `status: null` first. (2) **Pagination** — walk the RadGrid pager (or find a page-size arg) to exceed 10 results. (3) VIEWSTATE lifecycle per request. Sizing holds at `[L]`: core search is `[M]`, status-enrichment is the `[S–M]` tail.
